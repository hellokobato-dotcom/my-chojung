...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('🚀 직원 관리 시스템 v2.0 초기화 시작');
...
async function initializeSchema() {
    try {
        // 직원 테이블
        await updateTableSchema('employees', [
            { name: 'id', type: 'text', description: '직원 ID (UUID)' },
            { name: 'name', type: 'text', description: '직원 이름' },
            { name: 'department', type: 'text', description: '부서' },
            { name: 'phone', type: 'text', description: '연락처' },
            { name: 'employment_type', type: 'text', description: '고용 형태 (일용직/상용직)' },
            { name: 'hourly_rate', type: 'number', description: '시급' },
            { name: 'monthly_salary', type: 'number', description: '월급' },
            { name: 'bank_account', type: 'text', description: '계좌번호' },
            { name: 'hire_date', type: 'text', description: '입사일' },
            { name: 'resign_date', type: 'text', description: '퇴사일' },
            { name: 'status', type: 'text', description: '재직 상태 (재직/퇴직)' },
            { name: 'photo_url', type: 'text', description: '사진 URL' }
        ]);

        // 급여 지급 테이블
        await updateTableSchema('payments', [
            { name: 'id', type: 'text', description: '급여 ID (UUID)' },
            { name: 'employee_id', type: 'text', description: '직원 ID' },
            { name: 'employee_name', type: 'text', description: '직원 이름' },
            { name: 'employment_type', type: 'text', description: '고용 형태' },
            { name: 'payment_date', type: 'text', description: '지급일' },
            { name: 'payment_year', type: 'number', description: '지급 연도' },
            { name: 'payment_month', type: 'number', description: '지급 월' },
            { name: 'work_hours', type: 'number', description: '근무시간' },
            { name: 'hourly_rate', type: 'number', description: '시급' },
            { name: 'monthly_salary', type: 'number', description: '월급' },
            { name: 'base_salary', type: 'number', description: '기본급' },
            { name: 'deductions', type: 'number', description: '공제액' },
            { name: 'national_pension', type: 'number', description: '국민연금' },
            { name: 'health_insurance', type: 'number', description: '건강보험' },
            { name: 'long_term_care', type: 'number', description: '장기요양보험' },
            { name: 'employment_insurance', type: 'number', description: '고용보험' },
            { name: 'net_pay', type: 'number', description: '최종 지급액' },
            { name: 'business_account_pay', type: 'number', description: '사업용 계좌 지급액' },
            { name: 'personal_account_pay', type: 'number', description: '개인용 계좌 지급액' },
            { name: 'is_reported', type: 'text', description: '신고 여부' },
            { name: 'notes', type: 'text', description: '비고' }
        ]);
        
        console.log('✅ 테이블 스키마 초기화 완료');
    } catch (error) {
        console.error('❌ 테이블 스키마 초기화 실패:', error);
    }
}
...
async function loadEmployees() {
    try {
        ...
        const response = await fetch('tables/employees?limit=1000');
...
window.openEmployeeModal = function(employeeId) {
    console.log('📝 모달 열기:', employeeId ? '수정 모드' : '등록 모드');
    ...
    if (employeeId) {
        ...
        if (employee) {
            ...
            document.getElementById('resignDate').value = employee.resign_date || '';
            document.getElementById('employeeStatus').value = employee.status || '재직';
            ...
            document.getElementById('bankAccount').value = employee.bank_account || '';
            
            // 사진 표시
            if (employee.photo_url) {
                document.getElementById('photoUrl').value = employee.photo_url;
                document.getElementById('photoPreview').innerHTML = `<img src="${employee.photo_url}" alt="${employee.name}">`;
            } else {
                const initials = employee.name ? employee.name.substring(0, 2) : '?';
                document.getElementById('photoPreview').innerHTML = `<div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: 48px; font-weight: bold;">${initials}</div>`;
            }
            ...
            // 급여 지급 이력 표시
            loadEmployeePaymentHistory(employeeId);
            paymentHistorySection.style.display = 'block';
        }
    } else {
        // 등록 모드
        modalTitle.textContent = '직원 등록';
...
window.saveEmployee = async function() {
    ...
    try {
        ...
        if (employeeId) {
            ...
            const result = await response.json();
            savedEmployeeId = result.data.id;
        }
        ...
        if (hasPaymentInfo) {
            ...
            const paymentResponse = await fetch('tables/payments', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(paymentData)
            });
            
            if (!paymentResponse.ok) throw new Error('급여 저장 실패');
            
            alert(employeeId ? '직원 정보와 급여가 저장되었습니다.' : '새 직원과 급여가 등록되었습니다.');
        } else {
            alert(employeeId ? '직원 정보가 수정되었습니다.' : '새 직원이 등록되었습니다.');
        }
...
window.deleteEmployee = async function(employeeId) {
    ...
    try {
        ...
        // 관련 급여 내역 삭제
        const employeePayments = payments.filter(pay => String(pay.employee_id) === String(employeeId));
        for (const payment of employeePayments) {
            await fetch(`tables/payments/${payment.id}`, { method: 'DELETE' });
        }
